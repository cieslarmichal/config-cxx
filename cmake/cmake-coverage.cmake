...
# ~~~
#
# Example 3: Target added to the 'ccov' and 'config-ccov-all' targets
#
# ~~~
# add_code_coverage_all_targets(EXCLUDE test/*) # Adds the 'config-ccov-all' target set and sets it to exclude all files in test/ folders.
#
# add_executable(theExe main.cpp non_covered.cpp)
# target_code_coverage(theExe AUTO ALL EXCLUDE non_covered.cpp test/*) # As an executable target, adds to the 'ccov' and config-ccov-all' targets, and the reports will exclude the non-covered.cpp file, and any files in a test/ folder.
# ~~~
...
# config-ccov-all : Generates HTML code coverage report, merging every target added with 'ALL' parameter into a single detailed report.
#
# LLVM-COV:
# ccov : Generates HTML code coverage report for every target added with 'AUTO' parameter.
...
# config-ccov-all : Generates HTML code coverage report, merging every target added with 'ALL' parameter into a single detailed report.
# config-ccov-all-report : Prints summary per-file coverage information for every target added with ALL' parameter to the command line.
# config-ccov-all-export : Exports the coverage report to a JSON file.
#
# Required:
...
# ALL - Adds the target to the 'config-ccov-all' and 'config-ccov-all-report' targets, which merge several executable targets coverage data to a single report. Effective on executable targets.
        ...
        if(target_type STREQUAL "EXECUTABLE")
            if(CMAKE_C_COMPILER_ID MATCHES "(Apple)?[Cc]lang"
               OR CMAKE_CXX_COMPILER_ID MATCHES "(Apple)?[Cc]lang")
                ...
                # Run the executable, generating raw profile data Make the run
                # data available for further processing. Separated to allow
                ...
                # Export coverage information so continuous integration tools
                # (e.g. Jenkins) can consume it
                add_custom_target(
...
# Adds the 'config-ccov-all' type targets that calls all targets added via
...
# reports. Also adds the config-ccov-all-capture Generates an all-merged.info
